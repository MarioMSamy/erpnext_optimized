...
optimize_mariadb() {
    ...
    echo -e "${YELLOW}Optimizing MariaDB configuration based on server resources...${NC}"
    sleep 2
...
EOF"
    ...
    echo -e "${GREEN}MariaDB optimization complete!${NC}"
    sleep 2
}

# Function to optimize Python settings
optimize_python() {
    echo -e "${YELLOW}Optimizing Python settings...${NC}"
    sleep 2

    # Set environment variables for Python garbage collection
    export PYTHONOPTIMIZE=1
    export PYTHONGLOBAL=1
    export PYTHONFAULTHANDLER=1

    # Install optimized libraries
    pip3.10 install --upgrade pip
    pip3.10 install psutil pyperf

    echo -e "${GREEN}Python optimization complete!${NC}"
    sleep 2
}

# Function to optimize Node.js based on server resources
optimize_nodejs() {
    local total_ram=$(grep MemTotal /proc/meminfo | awk '{print $2}')
    local ram_mb=$((total_ram / 1024))

    echo -e "${YELLOW}Optimizing Node.js settings...${NC}"
    sleep 2

    # Calculate Node.js memory limit (50% of total RAM)
    local node_memory_limit=$((ram_mb * 50 / 100))
    ...
    echo -e "${GREEN}Node.js optimization complete!${NC}"
    sleep 2
}

# Function to optimize system-wide settings
optimize_system() {
    echo -e "${YELLOW}Optimizing system-wide settings...${NC}"
    sleep 2
...
EOF"
    ...
    if [[ $(lsblk -d -o rota | grep -c 0) -gt 0 ]]; then
        echo -e "${YELLOW}SSD detected. Optimizing disk I/O...${NC}"
    ...
    echo -e "${GREEN}System-wide optimization complete!${NC}"
    sleep 2
}

# Function to optimize ERPNext Bench
optimize_bench() {
    local cpu_cores=$(nproc)

    echo -e "${YELLOW}Optimizing ERPNext Bench settings...${NC}"
    sleep 2

    # Set worker counts based on CPU cores
    bench set-config background_workers $((cpu_cores * 2))
    bench set-config gunicorn_workers $((cpu_cores * 2))

    # Enable Redis caching
    bench set-config use_redis_cache 1
    bench set-config redis_cache redis://localhost:6379

    # Enable Redis for queues
    bench set-config use_redis_queue 1
    bench set-config redis_queue redis://localhost:6379

    # Enable Redis for socketio
    bench set-config use_redis_socketio 1
    bench set-config redis_socketio redis://localhost:6379

    # Restart bench to apply changes
    bench restart

    echo -e "${GREEN}ERPNext Bench optimization complete!${NC}"
    sleep 2
}
